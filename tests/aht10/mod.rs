...
fn trigger_reset_pass() {
    ...
    let computed = aht10.trigger_reset(&mut vbd);
...
fn trigger_calibration_pass() {
    ...
    let computed = aht10.trigger_calibration(&mut vbd);
    let expected = SensorState::Idle;

    assert_eq!(computed, expected);
    assert_eq!(vbd.lwb, [0xE1, 0x08, 0x00]);
}
...
fn get_sensor_status_not_initialized() {
    ...
    let computed = aht10.get_sensor_status(&mut vbd);
...
fn trigger_measurement_pass() {
    ...
    let computed = aht10.trigger_measurement(&mut vbd);
    let expected = SensorState::Idle;
