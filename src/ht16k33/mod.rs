...
pub fn set_blink_rate<Ibd>(ibd: &mut Ibd, did: u8, blink_rate: BlinkRate)
where
    Ibd: crate::traits::I2cBusDevice,
{
    let da = did::convert_did_to_address(did);
    hw::set_blink_rate(ibd, da, blink_rate as u8);
}
...
pub fn set_brightness_level<Ibd>(ibd: &mut Ibd, did: u8, value: u8)
where
    Ibd: crate::traits::I2cBusDevice,
{
    const MAX: u8 = 15;
    ...
    let da = did::convert_did_to_address(did);
    hw::set_brightness_level(ibd, da, value);
}

/// clear the display buffer and change to standby mode
pub fn clear<Ibd>(ibd: &mut Ibd, did: u8)
where
    Ibd: crate::traits::I2cBusDevice,
{
    let da = did::convert_did_to_address(did);
    // reset data
    hw::set_display_data(ibd, da, [0x00; 16]);
    // disable display
...
pub fn show_buffer<Ibd>(ibd: &mut Ibd, did: u8, buffer: [u8; 16])
where
    Ibd: crate::traits::I2cBusDevice,
{
    let da = did::convert_did_to_address(did);
...
pub fn show_number<Ibd>(ibd: &mut Ibd, did: u8, value: i32, digits: u8) -> bool
where
    Ibd: crate::traits::I2cBusDevice,
{
    const MIN: u8 = 2;
